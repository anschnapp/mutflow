...
object MutFlow {
    ...
    private val threadToSession = newConcurrentMap<Long, SessionId>()
...
/**
 * Generates a random UUID using the multiplatform `kotlin.uuid.Uuid` API.
 */
@OptIn(ExperimentalUuidApi::class)
internal fun newUuid(): Uuid = Uuid.random()

/**
 * Generates a seed value from the monotonic clock, usable on every backend.
 */
internal fun newSeed(): Long = TimeSource.Monotonic.markNow().elapsedNow().inWholeNanoseconds
