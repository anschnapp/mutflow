 ...
 *
 * Multiplatform: lives in `commonMain`; the only platform-specific pieces are
 * the concurrent collection factories ([newConcurrentSet], [newSynchronizedList])
 * and the monotonic clock (via [TimeSource.Monotonic], which is common).
 */
object MutationRegistry {
    ...
    fun checkTimeout() {
        ...
        val deadline = session.deadline
        if (deadline != null && deadline.hasPassedNow()) {
    ...
    fun <T> withSession(
        ...
        return withSessionLock(lock) {
            check(currentSession == null) { "Session already active" }
            val deadline = if (activeMutation != null && timeoutMs > 0) {
                TimeSource.Monotonic.markNow() + timeoutMs.milliseconds
            } else null
            currentSession = Session(activeMutation, deadline)
            try {
                val result = block()
                val session = currentSession!!
                result to SessionResult(
                    mutationPointCount = session.discoveredPoints.size,
                    discoveredPoints = session.discoveredPoints.toList()
                )
            } finally {
    ...
    private class Session(
        ...
        val deadline: TimeMark? = null,
        val discoveredPoints: MutableList<DiscoveredPoint> = newSynchronizedList(),
        val seenPointIds: MutableSet<String> = newConcurrentSet()
    )
}
